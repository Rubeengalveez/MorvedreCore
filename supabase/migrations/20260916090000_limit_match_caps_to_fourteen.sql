...
begin
  ...
  if new.cap_number not between 1 and 14 then raise exception 'El gorro debe estar entre 1 y 14.' using errcode='23514'; end if;
  if exists(select 1 from public.match_callups where match_id=new.match_id and player_id<>new.player_id and status in ('called','confirmed') and cap_number=new.cap_number) then
    raise exception 'Ese gorro ya está ocupado. Elige otro número.' using errcode='23514';
  end if;
  return new;
end; $$;
...
begin
  ...
  if supplied<1 or supplied>14
    ...
    or exists(select 1 from jsonb_array_elements(p_players) e where (e->>'cap')::integer not between 1 and 14 or e->>'cap' is null or not exists(select 1 from public.match_callups c where c.match_id=p_match and c.player_id=(e->>'id')::uuid and c.status in ('called','confirmed')))

-- Migración de Tiempos de Natación (swim_time_entries)
-- Aplica esta consulta en el SQL Editor de Supabase (Dashboard -> SQL Editor -> New query)

create table if not exists public.swim_time_entries (
...
drop policy if exists swim_time_entries_select_club_member on public.swim_time_entries;
...
drop policy if exists swim_time_entries_insert_coach on public.swim_time_entries;
...
drop policy if exists swim_time_entries_update_coach on public.swim_time_entries;
...
drop trigger if exists protect_swim_time_entry_changes on public.swim_time_entries;
...
drop trigger if exists audit_swim_time_entry_changes on public.swim_time_entries;

...
const cardVariants = cva("rounded-md border bg-paper-card text-ink-900", {
  variants: {
    variant: {
      ...
      hero: "border-ink-300 shadow-elev-2",
    },
    padding: {
      none: "",
      sm: "p-3",
...
const CardStripe = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { color: string }>(
...
>(({ className, ...props }, ref) => (
  <h3
    ref={ref}
    className={cn(
      "font-display text-xl font-bold leading-tight text-pool-deep",
      className,
    )}
    {...props}
  />

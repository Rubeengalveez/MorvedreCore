...
export function MyPositionSticky({
  ...
  return (
    <a
      href={href}
      data-my-position-sticky
      className={cn(
        "sticky top-[108px] z-[5] -mx-4 flex items-center gap-2.5 border-b border-ink-300 bg-paper/95 px-4 py-2 backdrop-blur supports-[backdrop-filter]:bg-paper/80 transition-colors hover:bg-pool-foam/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pool-blue focus-visible:ring-offset-2 focus-visible:ring-offset-paper",
        className,
      )}
    >
      {rank ? (
        ...
        <span
          aria-hidden="true"
          className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-pool-deep font-mono text-xs font-extrabold text-paper"
        >
          {row.position}
        </span>
      )}
      ...
      <div className="text-right">
        <p className="font-mono text-lg font-extrabold leading-none tabular-nums text-pool-deep">
          {row.primary_value}
          {metricSuffix}
        </p>
        <p className="text-[10px] font-bold uppercase tracking-eyebrow text-ink-600">

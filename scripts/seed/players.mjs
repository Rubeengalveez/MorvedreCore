...
const COACHES = [
  {
    name: "Vega Martínez",
    email: "vega.martinez@morvedre-core.test",
    gender: "female",
    teamLabels: ["Benjamín"],
  },
  {
    name: "Vitaliy Petrov",
    email: "vitaliy.petrov@morvedre-core.test",
    gender: "male",
    teamLabels: ["Alevín", "Infantil", "Cadete A", "Absoluto"],
  },
  {
    name: "Rubén Gálvez",
    email: "galvillo9@gmail.com",
    gender: "male",
    teamLabels: ["Cadete B", "Juvenil"],
    admin: true,
    playerTeam: "Absoluto",
  },
];
...
const DIRECTIVA = [
  {
    name: "Eva Sánchez",
    email: "eva.sanchez@morvedre-core.test",
    gender: "female",
    role_label: "Secretaria",
  },
  {
    name: "Mónica Gil",
    email: "monica.gil@morvedre-core.test",
    gender: "female",
    role_label: "Tesorera",
  },
  {
    name: "Sol Romero",
    email: "sol.romero@morvedre-core.test",
    gender: "female",
    role_label: "Tienda",
  },
  {
    name: "Álex Torres",
    email: "alex.torres@morvedre-core.test",
    gender: "male",
    role_label: "Área deportiva",
  },
  {
    name: "Laura Ferrer",
    email: "laura.ferrer@morvedre-core.test",
    gender: "female",
    role_label: "Área deportiva",
  },
  {
    name: "Miguel Navarro",
    email: "miguel.navarro@morvedre-core.test",
    gender: "male",
    role_label: "Área deportiva",
  },
];
...
async function createAuthUser(email) {
  ...
  if (error) throw error;
  users.set(email.toLowerCase(), data.user.id);
  return data.user.id;
}
...
async function main() {
  ...
  for (let i = 0; i < TEAMS.length; i++) {
    ...
    for (let p = 0; p < t.count; p++) {
      ...
      if (rand() < 0.85 && usedCaps.size < t.capMax) {
        do {
          cap = randInt(t.capMin, t.capMax);
        } while (usedCaps.has(cap));
        usedCaps.add(cap);
      } else {
        cap = null;
      }
    ...
    if (c.admin) {
      for (const role of ["admin", "player"]) {
        const { error } = await admin
          .from("user_roles")
          .upsert(
            { profile_id: id, role, scope_team_id: null, granted_by: null },
            { onConflict: "profile_id,role,scope_team_id" },
          );
        if (error) throw error;
      }
      dirIds.add(id);
      const playerTeamId = teamIdByLabel.get(c.playerTeam);
      if (playerTeamId) {
        const { error } = await admin
          .from("team_rosters")
          .upsert(
            { team_id: playerTeamId, player_id: id, squad_number: 7, joined_at: "2025-09-01" },
            { onConflict: "team_id,player_id" },
          );
        if (error) throw error;
        playerIds.add(id);
        const currentPlayers = playerIdByTeamLabel.get(c.playerTeam) ?? [];
        currentPlayers.push({ id, cap: 7, name: c.name, email: c.email });
        playerIdByTeamLabel.set(c.playerTeam, currentPlayers);
      }
    }
  }
  ...
  console.log("\n[players] Creando perfiles de padres/madres...");
  const parentIds = [];
  ...
  console.log("\n  Las cuentas demo usan contraseñas aleatorias únicas no persistidas.");
...
function seasonLabel(_seasonId) {
  return SEASON_LABEL;
}

function randomDateInSeason(_seasonId, fraction) {
